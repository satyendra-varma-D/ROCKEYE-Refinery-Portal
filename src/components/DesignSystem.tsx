...
export function Input({ label, error, className = "", ...props }: InputProps) {
  return (
    <div className="flex flex-col gap-1 w-full">
      ...
      <input
        className={`w-full px-3 py-1.5 bg-white border rounded-lg text-xs outline-none transition-all placeholder:text-slate-400 focus:border-blue-600 focus:ring-1 focus:ring-blue-600/30 disabled:bg-slate-50 disabled:text-slate-500 disabled:cursor-not-allowed ${
...
export function DataTable({
  ...
  const filteredData = data.filter((row) => {
    ...
    const matchesColumnFilters = activeColumns.every(colKey => {
      const filter = columnFilters[colKey]
      if (!filter || !filter.value) return true
      
      const rowVal = String(row[colKey] ?? row.details?.[colKey] ?? "").toLowerCase()
      const filterVal = filter.value.toLowerCase()
      
      switch (filter.operator) {
        case "contains": return rowVal.includes(filterVal)
        case "begins with": return rowVal.startsWith(filterVal)
        case "equals": return rowVal === filterVal
        case "is in": return filterVal.split(',').map(s => s.trim()).includes(rowVal)
        default: return true
      }
    })
                ...
                .map((col) => (
                  <th key={`filter-${col.key}`} className="px-2 py-2">
                    <div className="flex flex-col gap-1">
                      <select 
                        className="w-full text-[10px] border border-slate-200 rounded px-1 py-1 text-slate-500 focus:outline-none focus:border-blue-500"
                        value={columnFilters[col.key]?.operator || "contains"}
                        onChange={(e) => setColumnFilters(prev => ({
                          ...prev,
                          [col.key]: { ...prev[col.key], operator: e.target.value, value: prev[col.key]?.value || "" }
                        }))}
                      >
                      ...
                      <div className="relative">
                        <input 
                          type="text" 
                          className="w-full text-xs border border-slate-200 rounded px-2 py-1 focus:outline-none focus:border-blue-500 pr-6"
                          value={columnFilters[col.key]?.value || ""}
                          onChange={(e) => setColumnFilters(prev => ({
                            ...prev,
                            [col.key]: { ...prev[col.key], operator: prev[col.key]?.operator || "contains", value: e.target.value }
                          }))}
                        />
                      ...
                      if (col.key === "performanceTag") {
                        ...
                        return (
                          ...
                          >
                            ...
                            >
                              <span
                                className={`w-1.5 h-1.5 rounded-full ${tagDots[tag] || "bg-slate-400"}`}
                              />
                              {tag}
                            </span>
                          </td>
            ...
            >
              ...
              {!isCollapsed && (
                <div className="p-4 flex flex-col gap-3 bg-white">
                  {sectionFields.map((f) => {
                    const isIdField =
                      (f.key.toLowerCase() === "refno" ||
                        f.key.toLowerCase().endsWith("no") ||
                        f.key.toLowerCase().endsWith("code")) &&
                      !["zipcode", "postcode", "companycode"].includes(
                        f.key.toLowerCase(),
                      )
                    return (
                      <div key={f.key}>
...
export function DetailView({
  ...
  const getAssociatedRecords = (trxKey: string) => {
    ...
    const transaction = commercialMod?.transactions?.find(
      (t: any) => t.key === trxKey,
    )
      ...
      <div className="flex-1 flex flex-col bg-slate-50/50 overflow-hidden">
        ...
        <div className="flex-1 overflow-y-auto p-6 flex flex-col gap-6">
          ...
          <div className="bg-white rounded-xl border border-slate-200 p-5 flex flex-row items-center justify-between shadow-sm flex-shrink-0">
            ...
            {entity.code?.startsWith("ENQ-") &&
              entity.workflowStep !== "Accepted" &&
                entity.workflowStep !== "Rejected" && (
            ...
            {entity.code?.startsWith("QTN-") &&
              entity.details?.quotationNo &&
              entity.status !== "Approved" &&
                entity.status !== "Rejected" &&
                entity.workflowStep !== "Approved" && (
                <div className="flex items-center gap-2">
                  <Button
                    variant="primary"
                    icon="check"
                    className="bg-emerald-600 hover:bg-emerald-700 text-white font-bold px-3 py-1.5"
                    onClick={() =>
                      onAcceptQuotation && onAcceptQuotation(entity)
                    }
                  >
                    Accept & Create SO
                  </Button>
                  <Button
                    ...
                    onClick={() =>
                      onModifyQuotation && onModifyQuotation(entity)
                    }
                  >
                    Modify (New Version)
                  </Button>
                  <Button
                    ...
                    className="bg-red-50 hover:bg-red-100 text-red-600 font-bold px-3 py-1.5"
                    onClick={() =>
                      onRejectQuotation && onRejectQuotation(entity)
                    }
                  >
                    Reject
                  </Button>
                  ...
                  <tbody className="divide-y divide-slate-100 text-xs text-slate-700">
                    {enquiries.map((enq: any) => (
                      <tr key={enq.id} className="hover:bg-slate-50">
                        <td className="px-4 py-2.5 font-semibold text-blue-600 font-mono">
                          {enq.code}
                        </td>
                        <td className="px-4 py-2.5">{enq.details?.product}</td>
                        <td className="px-4 py-2.5 text-center font-mono">
                          {enq.details?.quantity}
                        </td>
                        <td className="px-4 py-2.5 text-center font-mono">
                          {enq.details?.targetDate}
                        </td>
                        <td className="px-4 py-2.5 text-right font-semibold text-slate-500">
                          {enq.workflowStep || enq.status}
                        </td>
                      </tr>
                    ))}
                    {enquiries.length === 0 && (
                  ...
                  <tbody className="divide-y divide-slate-100 text-xs text-slate-700">
                    {quotations.map((qtn: any) => (
                      <tr key={qtn.id} className="hover:bg-slate-50">
                        ...
                        <td className="px-4 py-2.5 text-center font-mono">
                          {qtn.details?.quantity}
                        </td>
                        <td className="px-4 py-2.5 text-right font-mono">
                          ₹{qtn.details?.unitPrice?.toLocaleString()}
                        </td>
                        <td className="px-4 py-2.5 text-right font-semibold text-slate-500">
                          {qtn.status}
                        </td>
                    ...
                    {quotations.length === 0 && (
                      <tr>
                        <td
                          ...
                          className="text-center py-8 text-slate-400 italic"
                        >
                          No associated quotations found.
                        </td>
                      </tr>
                    )}
                  </tbody>
                      ...
                      <tr key={so.id} className="hover:bg-slate-50">
                        ...
                        <td className="px-4 py-2.5 text-center font-mono">
                          {so.details?.requestedDeliveryDate ||
                            so.details?.startDate}
                        </td>
                        <td className="px-4 py-2.5 text-right font-semibold text-slate-500">
                          {so.workflowStep || so.status}
                        </td>
                      </tr>
                    ))}
                    {salesOrdersList.length === 0 && (
                      <tr>
                        <td
                          ...
                          className="text-center py-8 text-slate-400 italic"
                        >
                          No associated sales orders found.
                        </td>
                      </tr>
                    )}
                  </tbody>
