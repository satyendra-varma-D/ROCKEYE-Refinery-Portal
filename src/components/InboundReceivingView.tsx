...
// ── Types ──────────────────────────────────────────────────────────────────────
type WorkflowStatus = "yet_to_come" | "checked_in" | "quality_under_process" | "quality_hold" | "quality_rejected" | "checked_out_after_rejection" | "quality_approved" | "tank_allocated" | "grn_generated" | "checked_out_after_acceptance"
...
function getStepIndex(
  ...
  if (productType === "solid") {
    const map: Record<WorkflowStatus, number> = {
      ...
      quality_under_process: 1,
      quality_hold: 1,
      quality_rejected: 1,
      checked_out_after_rejection: 1,
      quality_approved: 1,
      tank_allocated: 1,
    }
    return map[status] || 0
...
function statusMeta(status: WorkflowStatus) {
  const map: Record<WorkflowStatus, {
    label: string
    color: string
    dot: string
    group: string
    ...
    checked_out_after_rejection: {
      label: "Checked Out (Rejected)",
      color: "bg-slate-100 text-slate-500",
      dot: "bg-slate-400",
      group: "completed",
    },
    checked_out_after_acceptance: {
      label: "Checked Out (Accepted)",
...
export function InboundReceivingView() {
  ...
  const [qcDecision, setQcDecision] =
    useState<"approved" | "rejected" | "hold" | null>(null)
  ...
  const step = selected
    ? getStepIndex(selected.status, selected.productType)
    : 0
  ...
  const isRejectedFlow =
    ...
    (selected.status === "quality_rejected" ||
      selected.status === "checked_out_after_rejection")

  const currentSteps = isSolidFlow
    ? STEPS_SOLID
    : isRejectedFlow
      ? STEPS_REJECT
      : STEPS_ACCEPT
  ...
  const submitQC = () => {
    if (!selected || !qcDecision)
      return alert("Please select Approve / Reject / Hold.")
    ...
    updatePlan(selected.id, {
      ...
      qualityResults: {
        ...
        documentName:
          uploadedDoc?.name || selected.qualityResults?.documentName,
      },
    })
  ...
  const allocateTank = () => {
    ...
    updatePlan(selected.id, {
      ...
      tankAllocation: {
        tankId: tank.id,
        tankName: tank.name,
        availableCapacity: tank.capacity - tank.current,
      },
    })
  ...
  const activePlans = plans.filter(
    (p) =>
      !["checked_out_after_acceptance", "checked_out_after_rejection"].includes(
        p.status,
      ),
  )
    ...
    <div className="flex h-full gap-4 min-h-0">
      ...
      <div className="w-72 shrink-0 flex flex-col gap-3 overflow-y-auto">
          ...
          <div className="grid grid-cols-2 gap-2 mb-1">
            <div className="text-center bg-slate-50 rounded py-2 border border-slate-100">
              <div className="text-sm font-bold text-slate-800">
                {plans.filter((p) => p.status === "yet_to_come").length}
              </div>
            ...
            <div className="text-center bg-violet-50 rounded py-2 border border-violet-100">
              <div className="text-sm font-bold text-violet-800">
                {plans.filter((p) => p.status === "checked_in").length}
              </div>
            ...
            <div className="text-center bg-amber-50 rounded py-2 border border-amber-100">
              <div className="text-sm font-bold text-amber-800">
                {
                  plans.filter((p) => p.status === "quality_under_process")
                    .length
                }
              </div>
            ...
            <div className="text-center bg-orange-50 rounded py-2 border border-orange-100">
              <div className="text-sm font-bold text-orange-800">
                {plans.filter((p) => p.status === "quality_hold").length}
              </div>
        ...
        <div className="flex flex-col gap-2">
          <h4 className="text-[10px] font-bold text-slate-500 uppercase tracking-wider px-1 mt-2">
            Active Vehicles
          </h4>
            ...
            return (
              ...
              >
                <div className="flex items-start justify-between gap-2 mb-2">
                  ...
                  <span
                    className={`inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-[9px] font-bold ${meta.color}`}
                  >
                ...
                <p className="text-[10px] text-slate-400 mt-0.5">
                  {plan.product} · {plan.quantity} MT
                </p>
              </button>
            )
          })}

          <h4 className="text-[10px] font-bold text-slate-500 uppercase tracking-wider px-1 mt-4">
            Checked Out
          </h4>
      ...
      {selected ? (
        <div className="flex-1 flex flex-col gap-4 min-h-0 overflow-y-auto pr-2 pb-8">
          ...
          <div className="bg-white rounded-xl border border-slate-200 shadow-sm p-5">
            <div className="flex items-center justify-between mb-1">
              <h2 className="text-sm font-bold text-slate-800">
                {selected.vehicleNumber} — {selected.vendor}
              </h2>
              <span
                className={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-[10px] font-bold ${statusMeta(selected.status).color}`}
              >
                <span
                  className={`w-1.5 h-1.5 rounded-full ${statusMeta(selected.status).dot}`}
                />
                {statusMeta(selected.status).label}
              </span>
            </div>
            <p className="text-[11px] text-slate-500 mb-5">
              {selected.product} · {selected.quantity} MT · PO: {selected.poRef}{" "}
            ...
            <div className="bg-white rounded-xl border border-slate-200 shadow-sm p-5">
              ...
              <div className="grid grid-cols-3 gap-4 mb-5 p-4 bg-slate-50 rounded-xl border border-slate-100">
                {[
                  ...
                  {
                    label: "Planned Qty (MT)",
                    value: `${selected.quantity} MT`,
                  },
              ...
              <div className="border-t border-slate-100 pt-4">
                <p className="text-xs font-bold text-slate-700 mb-3">
                  Record Vehicle Weights at Gate
                </p>
                ...
                <div className="flex justify-between items-center mt-6">
                  <p className="text-[10px] text-slate-400">
                    Recording weights moves vehicle to 'Checked In' status
                  </p>
                  <Button
                    variant="primary"
            ...
            <div className="bg-white rounded-xl border border-slate-200 shadow-sm p-5">
              ...
              <div className="grid grid-cols-4 gap-4 mb-6 p-4 bg-slate-50 rounded-xl border border-slate-100">
                {[
                  ...
                  {
                    label: "Gross Weight",
                    value: `${selected.gateEntry.grossWeight} MT`,
                  },
                  {
                    label: "Tare Weight",
                    value: `${selected.gateEntry.tareWeight} MT`,
                  },
                  {
                    label: "Net Weight",
                    value: `${selected.gateEntry.netWeight} MT`,
                  },
              ...
              {isSolidFlow ? (
                <div className="border-t border-slate-100 pt-5 mt-5">
                  <h4 className="text-xs font-bold text-slate-800 mb-3">
                    Allocate Warehouse & Generate GRN
                  </h4>
                  <div className="grid grid-cols-2 gap-4 mb-4">
                    {allWarehouses.map((w) => {
                      ...
                      const canFit =
                        available >=
                        (selected.gateEntry?.netWeight || selected.quantity)
                      const isSelected = selectedWarehouse === w.id
                      return (
                        <button
                          key={w.id}
                          onClick={() =>
                            setSelectedWarehouse(isSelected ? null : w.id)
                          }
                          disabled={!canFit}
                ...
                <div className="flex justify-between items-center border-t border-slate-100 pt-5">
                  <p className="text-[11px] text-slate-500">
                    Vehicle is waiting for QA lab to collect sample.
                  </p>
                  <Button
                    variant="primary"
                    onClick={initiateSampling}
                    icon="flask-conical"
                  >
                    Initiate Sampling
                  </Button>
            ...
            <div className="bg-white rounded-xl border border-slate-200 shadow-sm p-5">
              ...
              <p className="text-[11px] text-slate-400 mb-4">
                ...
                <strong className="text-slate-700">
                  {selected.sampling?.sampleId}
                </strong>{" "}
                · Collected At: {selected.sampling?.time}
              </p>
              ...
              {selected.qualityResults ? (
                ...
                <div>
                  <div className="grid grid-cols-5 gap-3 mb-4">
                    {QC_PARAMS.map((p) => {
                      const val = selected.qualityResults![
                        (p.key as keyof typeof selected.qualityResults)
                      ] as number
                      ...
                      return (
                        ...
                        >
                          <p className="text-[9px] text-slate-500 font-medium leading-tight mb-1">
                            {p.label}
                          </p>
                          <p
                            className={`text-base font-bold ${
                              pass ? "text-emerald-700" : "text-red-600"
                            }`}
                          >
                            {val}
                            {p.unit}
                  ...
                  <div className="flex items-center justify-between bg-slate-50 border border-slate-200 rounded-lg p-3 mb-4">
                    <div>
                      ...
                      <p className="text-xs font-medium text-slate-700">
                        {selected.qualityResults.remarks || "—"}
                      </p>
                    </div>
                    {selected.qualityResults.documentName && (
                    ...
                    <span
                      className={`px-3 py-1.5 rounded-full text-xs font-bold ${
                        selected.qualityResults.decision === "approved"
                          ? "bg-emerald-100 text-emerald-700"
                          : selected.qualityResults.decision === "rejected"
                            ? "bg-red-100 text-red-700"
                            : "bg-orange-100 text-orange-700"
                      }`}
                    >
                      Decision:{" "}
                      {selected.qualityResults.decision
                        .charAt(0)
                        .toUpperCase() +
                        selected.qualityResults.decision.slice(1)}
                    </span>
                  </div>
                    ...
                    <div className="p-4 bg-orange-50 border border-orange-200 rounded-xl flex items-center justify-between">
                      <div>
                        ...
                        <p className="text-[11px] text-orange-600">
                          This vehicle is awaiting re-test or management
                          approval. No further action can be taken right now.
                        </p>
                      </div>
                      <Icon
                        name="alert-circle"
                ...
                <div>
                  <div className="grid grid-cols-5 gap-3 mb-4">
                    {QC_PARAMS.map((p) => {
                      const val = parseFloat(
                        qcForm[(p.key as keyof typeof qcForm)] as string,
                      )
                  ...
                  <div className="grid grid-cols-2 gap-4 mb-4">
                    <div>
                      ...
                      <textarea
                        ...
                        onChange={(e) =>
                          setQcForm((f) => ({ ...f, remarks: e.target.value }))
                        }
                        rows={2}
                  ...
                  {!qcSubmitted ? (
                    ...
                    <div className="flex items-center gap-3 p-3 bg-slate-50 border border-slate-200 rounded-lg">
                      <p className="text-xs font-semibold text-slate-700 mr-auto">
                        Select Decision:
                      </p>
                      <button
                        onClick={() => {
          ...
          {step === 3 && selected.status === "quality_approved" && (
            <div className="bg-white rounded-xl border border-slate-200 shadow-sm p-5">
              <h3 className="text-sm font-bold text-slate-800 mb-1 flex items-center gap-2">
                <span className="w-6 h-6 rounded-full bg-teal-100 text-teal-700 flex items-center justify-center text-[10px] font-bold">
                  4
                </span>
                Pre-Discharge (Tank Allocation)
              </h3>
              <p className="text-[11px] text-slate-400 mb-4">
                Select an available tank for{" "}
                <strong className="text-slate-700">{selected.product}</strong> ·
                Net Qty:{" "}
                <strong className="text-slate-700">
                  {selected.gateEntry?.netWeight} MT
                </strong>
              </p>
              ...
              {selected.tankAllocation ? (
                <div className="flex items-center gap-3 p-4 bg-teal-50 border border-teal-200 rounded-xl">
                  ...
                  <div>
                    <p className="text-xs font-bold text-teal-700">
                      {selected.tankAllocation.tankName}
                    </p>
                    <p className="text-[10px] text-teal-600">
                      Allocated · Available Capacity:{" "}
                      {selected.tankAllocation.availableCapacity} MT
                    </p>
                  </div>
                </div>
              ) : (
                <>
                    ...
                    {availableTanksForProduct.map((tank) => {
                      ...
                      return (
                        <button
                          key={tank.id}
                          onClick={() =>
                            setSelectedTank(isSelected ? null : tank.id)
                          }
                          disabled={!canFit}
                          className={`text-left rounded-xl border p-3 transition-all ${
                            isSelected
                  ...
                  <div className="flex justify-between items-center mt-6">
                    <p className="text-[10px] text-slate-400">
                      Allocating a tank will mark Pre-Discharge as complete
                    </p>
                    <Button
                      variant="primary"
                      onClick={allocateTank}
                      icon="check-circle"
          ...
          {((step === 4 &&
            !isSolidFlow &&
            selected.status === "tank_allocated") ||
            (step === 2 &&
              isSolidFlow &&
              selected.status === "grn_generated")) && (
            <div className="bg-white rounded-xl border border-slate-200 shadow-sm p-5">
              <h3 className="text-sm font-bold text-slate-800 mb-4 flex items-center gap-2">
                <span className="w-6 h-6 rounded-full bg-blue-100 text-blue-700 flex items-center justify-center text-[10px] font-bold">
                  {isSolidFlow ? "3" : "5"}
                ...
                <div className="p-4 bg-emerald-50 border border-emerald-200 rounded-xl">
                  ...
                  <div className="grid grid-cols-4 gap-4">
                    {[
                      ...
                      {
                        label: "Received Qty",
                        value: `${selected.grn.receivedQty} MT`,
                      },
                      {
                        label: isSolidFlow
                          ? "Allocated Warehouse"
                          : "Allocated Tank",
                        value: isSolidFlow
                          ? selected.warehouseAllocation?.warehouseName || "—"
                          : selected.tankAllocation?.tankName || "—",
                      },
                ...
                <>
                  <div className="grid grid-cols-4 gap-4 mb-5 p-4 bg-slate-50 rounded-xl border border-slate-100">
                    {[
                      ...
                      { label: "Product", value: selected.product },
                      {
                        label: "Net Received Qty",
                        value: `${selected.gateEntry?.netWeight} MT`,
                      },
                      {
                        label: "Allocated Tank",
                        value: selected.tankAllocation?.tankName || "—",
                      },
                      { label: "PO Reference", value: selected.poRef },
                      {
                        label: "Gate Entry Ref",
                        value: selected.gateEntry ? "GE-" + selected.code : "—",
                      },
                      {
                        label: "Quality Document",
                        value: selected.qualityResults?.documentName || "—",
                      },
                  ...
                  <div className="flex justify-between items-center">
                    <p className="text-[11px] text-slate-400">
                      GRN will be auto-numbered and inventory will be updated.
                    </p>
                    <Button
                      variant="primary"
                      onClick={generateGRN}
            ...
            >
              ...
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-xs font-bold text-slate-800">
                    {selected.vehicleNumber} — {selected.driverName}
                  </p>
                </div>
                <button
          ...
          {[
            ...
            <div className="p-5 bg-slate-50 border border-slate-200 rounded-xl flex flex-col items-center justify-center text-center py-10 mt-4">
              ...
              <p className="text-[11px] text-slate-500">
                This vehicle completed its workflow (
                {selected.status === "checked_out_after_rejection"
                  ? "Rejected"
                  : "Accepted"}
                ) and left the facility at {selected.checkOut?.time}.
              </p>
            </div>
          )}
        </div>
        ...
        <div className="flex-1 flex items-center justify-center">
          <div className="text-center text-slate-400">
            ...
            <p className="text-sm font-medium">
              Select a vehicle to view workflow
            </p>
          </div>
        </div>
      )}
