...
export function ProcessTemplateView({ db, setDb }: ProcessTemplateViewProps) {
  ...
  const templateConfig = productionModule?.masters.find(
    (t) => t.key === "process-template",
  )
  ...
  const [formFinishedProduct, setFormFinishedProduct] = useState(
    "RBD Palm Olein (CP10)",
  )
  ...
  const handleStartEdit = () => {
    ...
    setFormFinishedProduct(
      selectedTemplate.details?.finishedProduct || "RBD Palm Olein (CP10)",
    )
    setFormVersion(selectedTemplate.details?.version || "1.0")
    setFormSteps(
      selectedTemplate.details?.steps
        ? [...selectedTemplate.details.steps]
        : [],
    )
    setIsEditing(true)
        ...
        auditTrail: [
          {
            timestamp: "Just now",
            user: "Admin",
            action: "Created Process Template",
          },
        ],
        activities: [
          {
            ...
            timestamp: "Just now",
            user: "Admin",
            description: `Template ${newTemplateCode} initialized`,
            type: "info",
          },
        ],
        comments: [],
  ...
  const addStep = () => {
    ...
    const nextInputProduct = lastStep
      ? lastStep.outputProduct
      : "Crude Palm Oil (CPO)"
  ...
  const currentStepDetails = stepCapacityAndBOM[activeStepName] || {
    minCap: 100,
    maxCap: 3000,
    bom: [{ material: "Feedstock Raw Material", ratio: 1.0, unit: "MT" }],
  }
  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      <div className="flex justify-between items-center">
        <div>
          ...
          <p className="text-[11px] text-slate-400">
            Define refining process flowcharts, stage BOM specifications, and
      ...
      {isEditing || isCreating ? (
        ...
        <form
          onSubmit={handleSave}
          className="bg-white border border-slate-200 rounded-xl p-6 shadow-xs flex flex-col gap-6"
        >
          <div className="flex justify-between items-center border-b border-slate-100 pb-4">
            <h3 className="text-sm font-bold text-slate-800">
              {isCreating
                ? "Create Refining Process Template"
                : `Edit Template: ${formTemplateName}`}
            </h3>
          ...
          <div className="flex flex-col gap-3">
            ...
            <div className="flex flex-col gap-3">
              {formSteps.map((step, idx) => (
                <div
                  key={idx}
                  className="flex items-center gap-3 bg-slate-50/50 p-4 border border-slate-150 rounded-xl relative group"
                >
                  ...
                  <div className="grid grid-cols-5 gap-3 flex-1">
                    ...
                    <div className="flex flex-col gap-1">
                      ...
                      <input
                        ...
                        onChange={(e) =>
                          handleStepChange(idx, "outputProduct", e.target.value)
                        }
                        required
                      />
                    </div>
              ...
              {formSteps.length === 0 && (
                <div className="text-center py-6 text-slate-400 text-xs italic w-full">
                  No refining steps defined. Click "Add Step" to configure the
                  process.
                </div>
              )}
            </div>
                ...
                return (
                  <button
                    ...
                    onClick={() => {
                      ...
                      const firstStep =
                        tmpl.details?.steps?.[0]?.name || "Degumming"
          ...
          <div className="col-span-3 flex flex-col gap-6">
            {selectedTemplate ? (
              <>
                ...
                <div className="bg-white border border-slate-200 rounded-xl p-5 shadow-sm flex justify-between items-center">
                  <div>
                    ...
                    <p className="text-[10px] text-slate-400 mt-0.5">
                      Code: {selectedTemplate.code} | Version:{" "}
                      {selectedTemplate.details?.version} | Created:{" "}
                ...
                <div className="bg-white border border-slate-200 rounded-xl p-6 shadow-sm flex flex-col gap-5">
                  <h3 className="text-xs font-bold text-slate-800 flex items-center gap-1.5 border-b border-slate-100 pb-3">
                    ...
                    Process Flowchart & Sequencing (Click step to analyze BOM &
                    capacity limits)
                  </h3>
                  ...
                  <div className="flex items-center gap-2 overflow-x-auto py-4 px-2">
                      ...
                      return (
                        <React.Fragment key={idx}>
                          ...
                          <div className="flex flex-col items-center flex-shrink-0 group">
                            ...
                            >
                              <div
                                className={`w-6 h-6 rounded-full text-[10px] font-bold flex items-center justify-center mb-1 ${
                                  isStepActive
                    ...
                    {steps.length === 0 && (
                      <div className="text-center py-6 text-slate-400 text-xs italic w-full">
                        No steps defined for this template. Click "Edit
                        Template" to add stages.
                      </div>
                    )}
                  </div>
                </div>
                  ...
                  <div className="col-span-1 flex flex-col gap-4 pr-6 border-r border-slate-100 justify-between">
                      ...
                      <div className="flex flex-col gap-3 text-xs">
                        ...
                        <div className="flex justify-between items-center py-1 border-t border-slate-50">
                          ...
                          <strong className="text-blue-700 font-mono">
                            {currentStepDetails.minCap} MT/Day
                          </strong>
                        </div>
                        <div className="flex justify-between items-center py-1 border-t border-slate-50">
                          <span className="text-slate-500">
                          ...
                          <strong className="text-red-750 font-mono">
                            {currentStepDetails.maxCap} MT/Day
                          </strong>
                        </div>
                      </div>
                    </div>
                    ...
                    <div className="bg-slate-50 p-4 border border-slate-150 rounded-xl flex flex-col gap-1 mt-4">
                      ...
                      <p className="text-[10px] text-slate-500 mt-1 leading-normal">
                        Operators use these thresholds to prevent refining
                        blockages or insufficient chemical dosage feedrates.
                      </p>
                    </div>
                  </div>
                      ...
                      <table className="w-full text-left text-xs border-collapse">
                        ...
                        <tbody className="divide-y divide-slate-100 text-slate-700 font-medium">
                          {currentStepDetails.bom.map((bomItem, index) => {
                            const minVal =
                              currentStepDetails.minCap * bomItem.ratio
                            const maxVal =
                              currentStepDetails.maxCap * bomItem.ratio
              ...
              <div className="bg-white border border-slate-205 rounded-xl p-8 text-center text-slate-400 italic text-xs">
                Please select a process template from the sidebar or click
                "Create Template".
              </div>
            )}
          </div>
