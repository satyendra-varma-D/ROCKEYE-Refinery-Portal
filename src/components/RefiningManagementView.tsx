...
export function RefiningManagementView({
  db,
  setDb,
}: RefiningManagementViewProps) {
  const [selectedWeek, setSelectedWeek] = useState("Week 1")

  // Load templates from database context
  const productionModule = db.find((m) => m.key === "production")
  const processTemplates =
    productionModule?.masters.find((m) => m.key === "process-template")
      ?.defaultData || []

  // Load production orders from database context dynamically
  const prodOrderTrx = productionModule?.transactions.find(
    (t) => t.key === "prodorder",
  )
  ...
  const salesOrders =
    commercialModule?.transactions.find((t) => t.key === "salesorder")
      ?.defaultData || []
  ...
  const [activeStepStatus, setActiveStepStatus] =
    useState<"Setup" | "Processing" | "On Hold" | "Completed" | "Dispatch">(
      "Setup",
    )
  ...
  const [dispatchMode, setDispatchMode] = useState<"packaging" | "tank" | null>(
    null,
  )
  ...
  const finishedProductTanks =
    tankMaster?.defaultData.filter(
  ...
  const paymentsReceived =
    financeModule?.transactions.find((t) => t.key === "paymentreceived")
      ?.defaultData || []
  ...
  const getSettlementStatus = () => {
    ...
    const payments = paymentsReceived.filter(
      (p) =>
        p.details?.salesOrderRef === linkedSo.code && p.status === "Cleared",
    )
    const totalPaid = payments.reduce(
      (acc, p) => acc + Number(p.details?.amount || 0),
      0,
    )
    ...
    if (paymentTerms === "100% Advance") requiredPercentage = 100
      ...
      requiredPercentage = 0 // Assuming letters of credit or CAD are verified outside

    const requiredAdvanceAmount = (totalRequired * requiredPercentage) / 100
    const isClearedForDispatch =
      totalPaid >= requiredAdvanceAmount || requiredPercentage === 0
  ...
  const handleConfirmTemplate = () => {
    ...
    defaultBOM.forEach((item) => {
      initialBOM[item.material] = Number(
        (clonedSteps[0].targetInputQty * item.ratio).toFixed(3),
      )
    })
  ...
  const handleProceedNext = () => {
    ...
    if (nextIdx >= executionSteps.length) {
      ...
      const finalStepOutput =
        executionSteps[executionSteps.length - 1]?.targetOutputQty ||
        activeOrder.details?.quantity ||
        100
      ...
      defaultBOM.forEach((item) => {
        nextBOM[item.material] = Number(
          (executionSteps[nextIdx].targetInputQty * item.ratio).toFixed(3),
        )
      })
    ...
    if (dispatchMode === "tank") {
      ...
      if (selectedTank) {
        ...
        if (dischargeQty > spaceRemaining) {
          alert(
            `Insufficient space in ${selectedTank.details?.tankName}. Available space: ${spaceRemaining.toFixed(1)} MT.`,
          )
          return
        }
        ...
        setDb((prevDb) =>
          prevDb.map((mod) => {
                      ...
                      return {
                        ...
                        details: {
                          ...
                          currentVolume: Number(
                            (currentVol + dischargeQty).toFixed(1),
                          ),
                        },
                      }
                    }),
            ...
            if (
              mod.key === "commercial" &&
              activeOrder.details?.salesOrderRef
            ) {
              return {
                ...mod,
                transactions: mod.transactions.map((trx) => {
                  ...
                  return {
                    ...
                    defaultData: trx.defaultData.map((so) => {
                      if (so.code !== activeOrder.details.salesOrderRef)
                        return so
                      return {
                        ...so,
                        status: "Executed",
        ...
        alert(
          `Discharged ${dischargeQty} MT refined product to ${selectedTank.details?.tankName}. Tank inventory and orders updated successfully.`,
        )
      }
    } else {
      ...
      alert(
        `Refined batch of ${dischargeQty} MT successfully routed directly to packaging queue. Mapped orders updated.`,
      )
    }
      ...
      <div className="flex justify-between items-center mb-5">
        <div>
          ...
          <p className="text-sm text-slate-500">
            Execute planned production orders week-wise step-by-step.
          </p>
        </div>
          ...
          <div className="col-span-1 bg-white border border-slate-200 rounded-xl p-4 flex flex-col gap-4 shadow-sm">
            ...
            <div className="flex flex-col gap-4 relative">
              {executionSteps.map((step: any, idx: number) => {
                const isCompleted =
                  idx < activeStepIdx || activeStepStatus === "Dispatch"
                const isActive =
                  idx === activeStepIdx && activeStepStatus !== "Dispatch"
                ...
                if (isCompleted) {
                  statusBadgeColor =
                    "text-emerald-700 bg-emerald-50 border-emerald-250 font-bold"
                  statusText = "Completed"
                } else if (isActive) {
                  if (activeStepStatus === "Processing") {
                    statusBadgeColor =
                      "text-blue-700 bg-blue-50 border-blue-250 font-bold animate-pulse"
                    statusText = "Processing"
                  } else if (activeStepStatus === "On Hold") {
                    statusBadgeColor =
                      "text-red-700 bg-red-50 border-red-250 font-bold"
                    statusText = "On Hold"
                  } else {
                    statusBadgeColor =
                      "text-blue-755 bg-blue-55/50 border-blue-200"
                  ...
                  <div key={idx} className="flex items-start gap-3 relative">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="mt-1 flex items-center gap-1.5">
                        <span
                          className={`text-[8px] px-1 rounded border font-semibold ${statusBadgeColor}`}
                        >
                          {statusText}
                        </span>
                      </div>
              ...
              <div className="flex items-start gap-3 relative">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="text-[9px] text-slate-400">
                    Packaging or Storage Tank
                  </div>
                  <div className="mt-1">
                    <span
                      className={`text-[8px] px-1 rounded border font-semibold ${
                        activeStepStatus === "Dispatch"
                          ? "text-blue-700 bg-blue-50 border-blue-250 font-bold"
                          : "text-slate-400 bg-slate-50 border-slate-200"
                      }`}
                    >
                      {activeStepStatus === "Dispatch"
          ...
          <div className="col-span-3 bg-white border border-slate-200 rounded-xl p-6 shadow-sm flex flex-col gap-6">
            {activeStepStatus === "Dispatch" ? (
                ...
                <div className="grid grid-cols-2 gap-4">
                  ...
                  >
                    ...
                    <span className="text-xs font-bold">
                      Move to Packaging Unit
                    </span>
                    <span className="text-[10px] text-slate-450">
                      Send yield directly to lines for filling, bottling, and
                      retail packaging.
                    </span>
                  </button>
                  ...
                  >
                    ...
                    <span className="text-[10px] text-slate-455">
                      Discharge yield into finished product tanks for bulk
                      holding.
                    </span>
                  </button>
                </div>
                        ...
                        >
                          ...
                          {finishedProductTanks.map((tank) => (
                            <option key={tank.id} value={tank.id}>
                              {tank.details?.tankName} (
                              {tank.details?.assignedProduct})
                            </option>
                          ))}
                        </select>
                      </div>
                      ...
                      <div className="flex flex-col gap-1">
                        ...
                        <div className="relative">
                          <input
                            ...
                            value={dischargeQty}
                            onChange={(e) =>
                              setDischargeQty(Number(e.target.value))
                            }
                          />
                          <span className="absolute right-3 top-1.5 text-[10px] text-slate-400 font-bold">
                            MT
                          </span>
                        </div>
                      </div>
                      ...
                      <div className="border-t border-slate-200 pt-3 flex flex-col gap-1.5 text-[11px] text-slate-600">
                        <div className="flex justify-between">
                          ...
                          <strong className="text-slate-800">
                            {selectedTank.details?.capacity} MT
                          </strong>
                        </div>
                        <div className="flex justify-between">
                          <span>Current Stock:</span>
                          <strong className="text-blue-600">
                            {selectedTank.details?.currentVolume} MT
                          </strong>
                        </div>
                        <div className="flex justify-between">
                          ...
                          <strong className="text-slate-800">
                            {(
                              selectedTank.details?.capacity -
                              selectedTank.details?.currentVolume
                            ).toFixed(1)}{" "}
                            MT
                          </strong>
                        </div>
                  ...
                  <div className="bg-slate-50 border border-slate-155 p-4 rounded-xl flex flex-col gap-3 animate-[slide-down_0.2s_ease-out]">
                    ...
                    {(() => {
                      ...
                      if (settlement) {
                        return (
                          <div
                            className={`p-3 rounded border text-[11px] font-medium flex flex-col gap-1.5 mt-1 ${
                              settlement.isClearedForDispatch
                          ...
                          >
                            ...
                            <div className="grid grid-cols-2 gap-2 mt-1">
                              ...
                              <div>
                                ...
                                <strong className="font-mono">
                                  ${settlement.totalRequired.toLocaleString()}
                                </strong>
                              </div>
                              <div>
                                ...
                                <strong className="font-mono">
                                  $
                                  {settlement.requiredAdvanceAmount.toLocaleString()}
                                </strong>
                              </div>
                              <div>
                                <span className="opacity-75">
                                  Total Cleared Payment:
                                </span>{" "}
                                <strong className="font-mono">
              ...
              <>
                ...
                {activeStepStatus === "Setup" && // SETUP BOM STEP
                (
                  <div className="flex flex-col gap-5">
                    <div className="bg-slate-50 border border-slate-150 p-4 rounded-xl">
                      <h4 className="text-xs font-bold text-slate-750">
                        Step Feedstock Specifications
                      </h4>
                      <div className="grid grid-cols-2 gap-4 mt-3 text-xs">
                        <div>
                          ...
                          of {activeStep?.inputProduct}
                        </div>
                        <div>
                          <span className="text-slate-400">
                            Projected Output Yield:
                          </span>{" "}
                          <strong className="text-slate-800 font-mono">
                            {activeStep?.targetOutputQty.toFixed(1)} MT
                          </strong>{" "}
                          of {activeStep?.outputProduct}
                        </div>
                      </div>
                    </div>
                    ...
                    <div className="flex flex-col gap-3">
                      <h4 className="text-xs font-bold text-slate-805">
                        Step Bill of Materials (BOM) Requirements
                      </h4>
                      ...
                      <div className="flex flex-col gap-3">
                        <div className="flex items-center gap-4 bg-white border border-slate-100 p-3 rounded-lg justify-between">
                          <div className="flex flex-col">
                            <span className="text-xs font-bold text-slate-800">
                              {activeStep?.inputProduct}
                            </span>
                            <span className="text-[10px] text-slate-400">
                              Main Feedstock (100% Volume)
                            </span>
                          </div>
                        ...
                        {(stepBOMDefinitions[activeStep?.name] || []).map(
                          (bomItem, idx) => (
                            <div
                              key={idx}
                              className="flex items-center gap-4 bg-white border border-slate-100 p-3 rounded-lg justify-between"
                            >
                              <div className="flex flex-col">
                                <span className="text-xs font-bold text-slate-800">
                                  {bomItem.material}
                ...
                {activeStepStatus === "Processing" && // PROCESSING
                (
                  <div className="flex flex-col items-center justify-center py-12 gap-5 text-center">
                    ...
                    <div className="flex flex-col gap-1">
                      <h4 className="text-sm font-bold text-slate-800">
                        Unit Processing Feedstock...
                      </h4>
                      <p className="text-[11px] text-slate-500 max-w-sm">
                        The refining unit is active. Feedstock inputs and
                        chemicals have been locked into the system. All other
                        stages are locked.
                      </p>
                    </div>
                    <div className="w-64 bg-slate-100 h-2 rounded-full overflow-hidden">
                      <div
                        className="h-full bg-blue-600 rounded-full animate-[pulse_1.5s_infinite]"
                        style={{ width: "65%" }}
                      />
                    </div>
                  </div>
                ...
                {activeStepStatus === "On Hold" && // ON HOLD
                (
                  <div className="flex flex-col items-center justify-center py-12 gap-5 text-center bg-red-50/20 border border-red-100 rounded-xl">
                    ...
                    <div className="flex flex-col gap-1">
                      <h4 className="text-sm font-bold text-slate-800">
                        Batch Processing On Hold
                      </h4>
                      <p className="text-[11px] text-red-550 max-w-sm">
                        Unit operations are suspended. Further processing is
                        blocked until the hold is resolved and batch operations
                        are resumed.
                      </p>
                    </div>
                  </div>
                ...
                {activeStepStatus === "Completed" && // COMPLETED
                (
                  <div className="flex flex-col gap-5">
                    <div className="flex flex-col items-center justify-center text-center py-6 gap-2">
                      ...
                      <p className="text-[11px] text-slate-400">
                        Step outputs generated successfully.
                      </p>
                    </div>
                      ...
                      <div className="grid grid-cols-3 gap-4 mt-3 text-xs font-medium">
                        ...
                        <div className="flex flex-col gap-0.5">
                          ...
                          <strong className="text-red-600 font-mono">
                            {(
                              activeStep?.targetInputQty -
                              activeStep?.targetOutputQty
                            ).toFixed(1)}{" "}
                            MT
                          </strong>
                        </div>
          ...
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="bg-slate-50 border-b border-slate-100">
                ...
                <th className="px-4 py-3 text-xs font-bold text-slate-500 uppercase tracking-wider text-right w-24">
                  Action
                </th>
              </tr>
            </thead>
                ...
                return (
                  ...
                  >
                    ...
                    <td className="px-4 py-3 text-slate-700">
                      {linkedSo ? (
                        linkedSo.details?.customer || linkedSo.name
                      ) : (
                    ...
                    <td className="px-4 py-3 text-slate-700 text-center font-mono">
                      {order.details?.quantity}
                    </td>
                    <td className="px-4 py-3 text-slate-750 text-center">
                      {order.details?.plannedWeek || selectedWeek}
                    </td>
                    <td className="px-4 py-3 text-slate-700 text-center font-mono">
                    ...
                    <td className="px-4 py-3 text-right">
                      {order.status === "Executed" ? (
                        <span className="text-[10px] font-bold text-slate-400 select-none mr-2">
                          Completed
                        </span>
                      ) : order.status === "Processing" ||
                        order.status === "Hold" ? (
          ...
          <div className="bg-white rounded-xl shadow-xl border border-slate-200 p-5 max-w-sm w-full flex flex-col gap-4 animate-[slide-up_0.25s_ease-out]">
            <div>
              <h3 className="text-sm font-bold text-slate-800">
                Select Refining Process Template
              </h3>
              <p className="text-[10px] text-slate-400 mt-0.5">
                Choose which refining configuration template to follow for
                Production Order{" "}
                {prodOrders.find((o) => o.id === pickingOrderId)?.code}.
              </p>
            </div>
